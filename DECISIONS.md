...
**¿Por qué ahí?**
Porque quise aplicar los principios de *Domain-Driven Design (DDD)*. Las reglas que definen qué estado puede seguir a otro son el corazón del negocio. Si ponía esa validación suelta en un Controlador o en un Servicio, corría el riesgo de que mañana otro desarrollador creara un script, un cronjob o un nuevo endpoint que se saltara la regla. Al encapsular la validación *dentro* del objeto mismo (mediante su método `cambiarEstado()`), hago matemáticamente imposible que un inmueble adquiera un estado inválido, sin importar desde dónde se intente modificar en la aplicación.
...
- **Sin rotación de Tokens (Refresh Tokens):** El sistema de autenticación usa un único *Access Token*. No implementé la mecánica de *Refresh Tokens* para mantener la sesión viva a largo plazo sin comprometer la seguridad.
- **Dockerización (Docker Compose):** Aunque el proyecto es muy fácil de levantar, me hubiese gustado armar un `docker-compose.yml` que orquestara PostgreSQL, el backend y el frontend con un solo comando para la evaluación.
